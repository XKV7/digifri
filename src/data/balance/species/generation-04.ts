...
export function initGenerationFour(): SpeciesDataMapConfig {
  ...
  // Adeus - a wholly original, non-canon entity created for this fork (see SpeciesId.ADEUS's own
  // doc comment). Fought only in its own Mystery Encounter; winning permanently unlocks it as a
  // cost-10 starter (Unlockables.ADEUS) rather than being caught in the normal sense.
  generationFourSpeciesData[SpeciesId.ADEUS] = {
    species: new PokemonSpecies({
      id: SpeciesId.ADEUS,
      generation: 4,
      mythical: true,
      category: "Singularity Pokémon",
      type1: PokemonType.DRAGON,
      type2: PokemonType.PSYCHIC,
      height: 3.5,
      weight: 350,
      ability1: AbilityId.SINGULARITY,
      ability2: AbilityId.NONE,
      abilityHidden: AbilityId.NONE,
      baseTotal: 1440,
      baseHp: 240,
    ...
    starter: SpeciesId.ADEUS,
    starterCost: 10,
    evolutions: [],
    eggTier: EggTier.LEGENDARY,
    passives: AbilityId.EVENT_HORIZON,
    // The union of Dialga/Palkia/Giratina's, Necrozma's (base form + its Ultra-form-exclusive
    // Sunsteel Strike/Moongeist Beam, see generationSevenSpeciesData[SpeciesId.NECROZMA] in
    // generation-07.ts), and Cosmoem's own level-up movesets, at whichever level the lowest of
    // them would learn each move (Necrozma's own two Relearn-only Morning Sun/Moonlight entries
    // are skipped in favor of Cosmoem's stronger Evolve-move versions of the same two moves).
    levelMoves: [
  ...
  // Arceus#0 - a wholly original, non-canon entity created for this fork: Arceus's own "true
  // form" as a standalone Pokemon rather than a held-item-dependent variant (see SpeciesId.ARCEUS_
  // ZERO's own doc comment). Its `forms` array mirrors real Arceus's 18 type-forms exactly (same
  // formKey/type1 per index, so the shared Legend Plate retyping logic in legend-plate.ts - which
  // indexes `pokemon.species.forms` by PokemonType - works identically) but every one of them
  // visually resolves to real Arceus's own base "normal" sprite/icon/cry regardless of which form
  // is actually active (see the SpeciesId.ARCEUS_ZERO special-casing in getBaseSpriteKey()/
  // getIconId()/getCryKey() below) - a brightness boost (see Pokemon#init() in field/pokemon.ts)
  // is its only visual distinction from a real Arceus. legend-plate.ts's own `||` special-casing
  // makes it always benefit from the retyping effect, without needing to actually hold a Legend
  // Plate.
  generationFourSpeciesData[SpeciesId.ARCEUS_ZERO] = {
    species: new PokemonSpecies({
      id: SpeciesId.ARCEUS_ZERO,
      generation: 4,
      mythical: true,
      category: "True Self Pokémon",
      type1: PokemonType.NORMAL,
      type2: null,
      height: 3.2,
      weight: 320,
